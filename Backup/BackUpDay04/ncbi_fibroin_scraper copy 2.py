...
# The final result structure will now be keyed by Family -> Organism -> Sequence Type -> Chain Type
# {Family: {Organism: {'full sequence': {'heavy chain': [data...], ...}, 'partial sequence': {...}}}}
...
def extract_organism_name(protein_name: str) -> str:
    ...
    Example: 'Fibroin heavy chain [Hesperophylax occidentalis]' -> 'Hesperophylax occidentalis'
    
    If the name is not found in brackets, it returns a default "Unknown Organism".
    """
    ...
    if match:
        ...
        # Ensure the extracted name is meaningful (e.g., not just an abbreviation)
        if len(organism) > 3:
            return organism
    
    # Fallback if brackets are missing or content is too short
...
def fetch_protein_sequence(accession_id: str) -> str:
    ...
    try:
        ...
        # Remove any non-standard amino acid characters (like 'X' or 'B' sometimes used as placeholders)
...
def main_scraper() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """
    Executes the full scraping process based on the nested taxonomy structure.
    Returns a classified dictionary and the taxonomy path map.
    The new classification includes the Organism name.
    """
    ...
    for family in families_to_process:
        ...
        for record_id, record_name in protein_records:
            ...
            # 0. Extract Organism Name (NEW STEP)
            ...
            sequence_data = {
                'id': record_id,
                'name': record_name,
                'organism': organism_name, # Include organism in data
                'sequence': sequence
            }
        ...
        # Calculate counts for the family by iterating over all organisms in it
        full_count = sum(
            len(v) 
...
def generate_summary_index(results: Dict[str, Any], root_dir: str):
    ...
    content = [
        ...
        "**NOTE:** The output files are now nested by Family, then by **Organism** for finer organization.",
        ""
    ]
...
def save_results_to_files(results: Dict[str, Any], path_map: Dict[str, Any]):
    """
    Creates the complex nested folder structure (Suborder/Superfamily/Family/Organism/SeqType/ChainType) 
    ...
    for family, organisms_data in results.items():
        ...
        # --- 1. Create Top Level Taxonomy directories (Suborder/Superfamily) ---
        ...
        # --- 2. Iterate through Organism, Sequence Type and Chain Type (New Levels) ---
        for organism_name, org_data in organisms_data.items():
            
            # Path: Family/Organism (NEW LEVEL)
            organism_dir = family_dir / safe_filename(organism_name, 60) # Allow longer names for species
