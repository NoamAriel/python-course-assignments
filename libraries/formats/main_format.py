# libraries/main_format.py
# This script uses the libraries ncbi_protein_scraper_lib.py, serine_sxn_analysis_lib.py and serine_sxn_plot_lib.py
# to scrape NCBI protein records, analyze [SX]n motifs,
# and generate plots based on specified filters.

# """
# Friendly wrapper to run serine_sxn_plot_lib with common filters.
# Adjust the settings below, then run:
#     python main_format.py
# Filter notes (all optional unless noted):
# - taxonomy_terms: one or more lineage names; keeps records if any match (ci).
# - protein_types: one or more substrings matched against `type` (ci).
# - partial_full: "full" (default), "partial", or None to disable this filter.
# - length_range: (MIN, MAX) inclusive; set to None to skip.
# - length_threshold + length_mode: keep lengths >= or <= threshold.
# - longest_factor: float > 1; after other filters, keep lengths >= longest/ factor.
# """
